...
### /demographicsData/demographicsStructure/
...
for year in periodicity:
    dictURL['{0}'.format(year)] = 'https://raw.githubusercontent.com/epicalekspwner/OpenDataCustomerAnalytics/main/demographicsData/demographicsStructure/' + str(year) + '_demographicsStructure_Absolute.csv'
...
for i in dictDataFrames.keys():
    exportPath = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsData/demographicsStructure/'
    exportPathExist = os.path.exists(exportPath)
    if not exportPathExist:
        os.makedirs(exportPath)
    exportFile = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsData/demographicsStructure/{0}_demographicsStructure_CantonRelative.csv'.format(i)
