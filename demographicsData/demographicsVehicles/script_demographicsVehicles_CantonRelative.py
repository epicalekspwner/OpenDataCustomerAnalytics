...
### /demographicsData/demographicsVehicles/
...
for year in periodicity:
    dictURL['{0}'.format(year)] = 'https://raw.githubusercontent.com/epicalekspwner/OpenDataCustomerAnalytics/main/demographicsData/demographicsVehicles/' + str(year) + '_demographicsVehicles_Absolute.csv'
...
for i in dictDataFrames.keys():
    exportPath = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsData/demographicsVehicles/'
    exportPathExist = os.path.exists(exportPath)
    if not exportPathExist:
        os.makedirs(exportPath)
    exportFile = 'C:/GitHub/OpenDataCustomerAnalytics/demographicsData/demographicsVehicles/{0}_demographicsVehicles_CantonRelative.csv'.format(i)
